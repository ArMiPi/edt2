...
Form newForm(String command) {
    ...
    if(strcmp(splt[0], "c") == 0) frm->protection = 60;
    else if(strcmp(splt[0], "r") == 0) frm->protection = 60;
    else if(strcmp(splt[0], "l") == 0) frm->protection = 50;
    else if(strcmp(splt[0], "t") == 0) frm->protection = 5;

    for(int i = 0; splt[i] != NULL; i++) free(splt[i]);
    free(splt);
